...
public class BeanCobros implements Serializable {
    ...
    public BeanCobros() {
        totalseleccionado= BigDecimal.ZERO; 
    ...
    private BigDecimal totalseleccionado, intereses, vuelto, totalpagar;
    ...
    public void setTotalseleccionado(BigDecimal totalseleccionado) {
 ...
 /*       for (int i = 0; i < listaCobrosCheques.size(); i++) {
    ...
    public void AceptarEditarCuentasCobrar(){
        ...
        if(cuentacobrar.cuentacobrar.getImporte()!= BigDecimal.ZERO){
    ...
    public void AceptarCobroTarjeta(){
        ...
        if(cobrotarjeta.getMontoTarjeta().equals( BigDecimal.ZERO)){
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN,"Monto debe ser mayor a 0",""));
            RequestContext.getCurrentInstance().update("formTarjeta");
        }else if((BigDecimal.ZERO.subtract(vuelto)).compareTo(cobrotarjeta.getMontoTarjeta())==-1){           
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN,"Monto Tarjeta no puede pasar el monto de "+Metodos.FormatoNumero(BigDecimal.ZERO.subtract(vuelto)),""));
            RequestContext.getCurrentInstance().update("formTarjeta");        
  ...
  /*      if(cobrocheque.getMontoCheque().equals( BigDecimal.ZERO)){
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN,"Monto debe ser mayor a 0",""));
            RequestContext.getCurrentInstance().update("formCheque");
        }else if((BigDecimal.ZERO.subtract(vuelto)).compareTo(cobrocheque.getMontoCheque())==-1){           
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN,"Monto Cheque no puede pasar el monto de "+Metodos.FormatoNumero(BigDecimal.ZERO.subtract(vuelto)),""));
            RequestContext.getCurrentInstance().update("formCheque");        
    ...
    public void AceptarCobroEfectivo(){
        if (cobroformadet.getMontoDet().equals(BigDecimal.ZERO)) {
    ...
    public void Grabar(){
        try{
            ...
            if(totalpagar.compareTo(BigDecimal.ZERO)==0){
                ...
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN,"Aun tiene saldo pendiente de "+Metodos.FormatoNumero(BigDecimal.ZERO.subtract(vuelto)),""));
                RequestContext.getCurrentInstance().update("formCobros:gCobros"); 
            }else{
                ...
                for (int i = 0; i < listaCuentasCobrarPendiente.size(); i++) {
                    if(listaCuentasCobrarPendiente.get(i).getSeleccionado()==true){
                        ...
                        if(listaCuentasCobrarPendiente.get(i).cuentacobrar.getImporte().compareTo(listaCuentasCobrarPendiente.get(i).cuentacobrar.getSaldo())==-1){
                            BigDecimal result = listaCuentasCobrarPendiente.get(i).cuentacobrar.getSaldo().subtract(listaCuentasCobrarPendiente.get(i).cuentacobrar.getImporte());
                            ...
                            listaCuentasCobrarPendiente.get(i).cuentacobrar.setSaldo(BigDecimal.ZERO);
                            listaCuentasCobrarPendiente.get(i).cuentacobrar.setImporte(BigDecimal.ZERO);
                            listaCuentasCobrarPendiente.get(i).cuentacobrar.setEstado("P");
                            CuentasCobrarFacade.edit(listaCuentasCobrarPendiente.get(i).cuentacobrar);
                        } 
                    }
                }
                BigDecimal totalcheque= BigDecimal.ZERO, totaltarjeta= BigDecimal.ZERO;
            ...
            //        totalcheque= totalcheque.add(listaCobrosCheques.get(i).getMontoCheque());
                ...
                if(totalcheque.compareTo(BigDecimal.ZERO)==1){ 
                ...
                if(totaltarjeta.compareTo(BigDecimal.ZERO)==1){
                    cobroformadet_aux = new CobrosFormasDet();
